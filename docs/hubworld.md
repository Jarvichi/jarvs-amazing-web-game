...
2. **Add one interactable per town** at that tile's `tx`/`ty`, with a
   ...
   **The difficulty rides in the screen id** — `hub-casks` / `-cellar` /
   `-vault`, exactly as fishing carries `hub-fishing-cave` / `-lake` /
   `-ocean` — so a town's tier is authored in its config, not chosen by the
   player in a menu.
3. **Register the screens** in `app/screens.ts`, a lazy export in
   `app/lazyScreens.ts`, and routes in `app/routes/HubRoutes.tsx` wrapped in
   `OverlayScreen`. Grant the rewards in the route's `onDone`.
4. **Gate it in `HubWorld.tsx`'s `handleNodeInteract`**, beside the existing
   rod/bait and crank checks: a `screen.startsWith(...)` test for the tool, then
   one for the per-town daily cooldown. Add `SCREEN_ENTER_LABEL` entries.
5. **Persist the cooldown** in a `game/hub/<thing>.ts` copied from `digs.ts` —
...
### Content coverage: hand-authored pilot + auto-generated for everyone else

Every named NPC with a `dialogue` array (216 across all 13 towns) has
`conversationTopics` today, in two tiers:

1. **Hand-authored pilot (Ravenwatch)** — five flavor-rich exterior NPCs got a
   full bespoke 3-topic pass as the reference implementation, each ending
   written specifically for that NPC's established lore: **The Elder**
   (`elder`: `elder-topic-pendant`, `elder-topic-before-fracture`,
   `elder-topic-roads`), **Vex the Merchant** (`merchant`:
   `merchant-topic-routes`, `merchant-topic-caravans`, `merchant-topic-trust`),
   **Old Greyfish** (`fisherman`: `fisherman-topic-water`,
   `fisherman-topic-glowfish`, `fisherman-topic-patience`), **Vince the
   Dealer** (`dealer`: `dealer-topic-advice`, `dealer-topic-bigwin`,
   `dealer-topic-skeptic`), and **Innkeeper Rosie** (`innkeeper-rosie`:
   `innkeeper-rosie-topic-business`, `innkeeper-rosie-topic-selfcare`,
   `innkeeper-rosie-topic-history`). Use these as the model when you have time
   to hand-write a full bespoke pass for a specific NPC.

2. **Auto-generated (every other named NPC — 211 NPCs, 447 topics)** — writing
   bespoke branches for all 216 NPCs by hand isn't practical in one pass, so
   the rest were generated from data each NPC already has: up to 3 topics per
   NPC, one per (de-duped) line in that NPC's own existing `dialogue` array,
   capped at 3. The seed line becomes the topic's `start` node text verbatim
   (so the topic is still tied to something that specific NPC actually says)
   and its button `label` is a short quoted excerpt of that line (`💬 "…"`).
   The **branching reaction structure below the seed line is a fixed,
   NPC-agnostic template** — identical tone-choice labels, reaction text, and
   deltas (`+8`/`+4 ally` good, no-effect neutral, `-8`/`-3` bad) on every
   auto-generated topic — since writing unique reaction prose at this scale
   wasn't feasible. Ids follow `<town>-<npcId>-topic-auto-<n>` (town-prefixed
   because `HUB_CONVERSATION_TOPICS`/`HUB_DIALOGUES` are merged globally
   across every town's bundle — see loader.ts — so ids must be globally
   unique even though NPC ids are only scoped per-town). Generated by a
   one-off script (not checked in); re-running it for a newly-added NPC would
   need the same seed-from-`dialogue` + template approach reimplemented, or a
   hand-authored pass following the pilot above instead.

Replacing an auto-generated NPC's topics with a bespoke hand-authored pass
later is just steps 1–4 of the checklist above, overwriting that NPC's
existing `conversationTopics`/`dialogues` entries.

### Friendship reaction (❤️/💔 above the NPC's head)

Whenever a friendship grant actually changes the value (nonzero xp — up from
a good ending, gift, or quest reward; down from a bad ending or a disliked
gift), a heart (❤️, xp > 0) or broken heart (💔, xp < 0) floats up and fades
above that NPC's head for about a second, if they're currently a visible
**exterior** named-NPC sprite (indoors, gated-off, or off-screen NPCs — e.g. a
quest/bounty reward or a dialogue-tree effect naming a *different* NPC than
the one you're talking to — just skip the effect; nothing errors).

- All friendship grants in `HubWorld.tsx` go through `grantFriendship(npcId, xp)`
  (near `grantQuestReward`) instead of calling `addFriendshipXp` directly —
  it grants the XP and then fires the reaction. Any new code that grants
  friendship should call `grantFriendship`, not `addFriendshipXp`, so the
  visual stays consistent.
- The reaction itself lives in `HubTownCanvas.tsx`: `doShowFriendshipReaction`
  (registered into the `showFriendshipReactionRef` prop, the same
  imperative-callback pattern as `moveInteractableRef`/`markInteractableSoldRef`)
  spawns a `PIXI.Text` emoji into `activeReactions`, ticked every frame
  (rises ~20px, fades over the back 40% of `REACTION_MS` = 1100ms) alongside
  the existing idle-speech-bubble (`activeBubbles`) loop, and removed at once
  if the NPC walks indoors mid-flight.
- Bounty-reward friendship (`turnInBounty` in `game/hub/bounties.ts`) is
  **not** wired to this — bounty turn-in happens at the Bounty Board modal,
  decoupled from any NPC sprite, so there's no head to float a heart above.
...
Weather content: rain puddles (Millhaven — rains year-round — plus
...
| `x`, `y` | `number` | Grid cell of the piece's top-left corner, 0-indexed, bounded by `HOME_GRID_COLS` / `HOME_GRID_ROWS` (currently 9×7 — odd on purpose, see §19). |
...
`HomeShelf` (the `home-shelf`/`home-shelf-decorate` screens, reached with
only an `onBack` prop plus `houseKey`/`initialTab` — no new `App.tsx`
routing needed beyond the existing pair) has a **SHELF / DECORATE / ROOMS**
tab row (reusing `HallOfAchievements`/`TownJournal`'s `.hoa-tabs`/`.hoa-tab`
CSS; ROOMS is hidden when `houseKey === 'default'`, i.e. no real house
context — see §19's "Room Slots"). SHELF is the original read-only
relic/keepsake display, unchanged. DECORATE renders the grid, using three
pure-visual pieces under `web/src/components/hub/home-shelf/` (each with a
`.stories.tsx`):
...
**`houseKey` is always the main room's key** (`${town}:${mainBuildingId}`)
— `HubWorld.tsx`'s 🛋 DECORATE button resolves this even when the player is
standing inside a purchased room slot, so it never accidentally scopes a
room-slot purchase to a sub-room's key. **Room selector**: when the house
has any purchased slots, DECORATE shows an `.hoa-tabs` row above the grid
— "Main Room" plus one button per purchased slot (from `getRoomSlotDef`) —
switching calls `loadHomeLayout` for that room's own key
(`${houseKey}-${slotId}`, matching the interior id `houseRooms.ts`
synthesizes) and resets any armed/selected piece. Each room's furniture is
a fully independent `PlacedFurniture[]` layout; there is no single
"current room" — `selectedRoomId` local state picks which key every
`placeFurniture`/`moveFurniture`/`removeFurniture` call in the tab targets.
...
- **Room size must match the DECORATE grid.** The walkable floor of an
  ...
  `HOME_GRID_COLS × HOME_GRID_ROWS` = **9×7** (`homeLayout.ts`) — odd on
  purpose, so the outer `width`/`height` (`HOME_GRID_COLS+2 ×
  HOME_GRID_ROWS+2` = **11×9**) are odd too, centering symmetric features
  (the front door, the rear-room passage) on a single tile. A `playerDecor`
  interior's `width`/`height` should always be `HOME_GRID_COLS+2 ×
  HOME_GRID_ROWS+2` — anything smaller clips placed furniture into (or
  entirely off) the walkable floor; `houseRooms.ts`'s `synthesizeSlotInterior`
  derives this from the constants directly rather than hardcoding it, so a
  future resize is a two-constant change. The three live player houses all
  use 11×9 (hand-authored per town, so their `config.json` numbers need
  updating in lockstep with `HOME_GRID_COLS`/`HOME_GRID_ROWS` if that ever
  changes again).
...
11×9 interior) — mirrored docking positions, same convention as any other
...
- If `HUB_INTERIORS[buildingId]` misses, checks whether `buildingId` is
  ...
  `HubInterior` (11×9, `playerDecor: true`, one `roomExit`) from the
...
- **`currentInteriorObj` caches the resolved interior.** Anything that needs
  "the interior currently being stood in" (the tap-to-walk handler, in
  particular) must read this cached value, never re-derive it via
  `HUB_INTERIORS[currentInteriorId]` — that map only has statically
  authored rooms, so doing so silently breaks every synthesized room-slot
  interior (#1949 — buying a room worked, walking into it worked, but every
  tap inside it, including the one back through the exit, silently no-op'd).
