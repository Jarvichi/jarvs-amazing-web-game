...
— it has no UI and does not depend on a furniture catalog, since neither
exists yet (tracked separately, see below).
...
| `itemId` | `string` | Furniture catalog id. Stored as an opaque string — this store does not validate it against a catalog. |
| `x`, `y` | `number` | Grid cell, 0-indexed, bounded by `HOME_GRID_COLS` / `HOME_GRID_ROWS` (currently 8×6). |
| `rotation` | `0 \| 90 \| 180 \| 270` | Facing/orientation of the placed piece. |
...
| `placeFurniture(itemId, x, y, rotation?)` | Adds a new placement. Returns `null` if `x`/`y` is out of bounds or the cell is already occupied. |
| `moveFurniture(id, x, y, rotation?)` | Relocates an existing placement. Returns `false` if the id is unknown, the target is out of bounds, or occupied by a *different* piece (moving onto its own current cell is allowed). |
...
error, matching every other hub store.

### Dependencies not yet built

- **Furniture catalog** (item defs — name, icon, footprint, cost/source):
  not yet implemented. Until it lands, `itemId` is unvalidated.
- **Decoration UI**: not yet implemented. Will extend `HomeShelf.tsx` with
  ...
  and renders `loadHomeLayout()` on a grid.
