...
**Target pool size: one quest chain per act** (28 total, one per `defeat_boss` act id — including both finales). Every act should have exactly one chain whose final step is `{ "type": "defeat_boss", "actId": "<that act's id>" }`, themed to that act's `rewardTags`/environment, with an intro line that fits the story beat at that point in the arc (read `docs/campaign2.md` before authoring arc2 chains). The target card should be a card already introduced by or thematically fitting that act — reuse an existing `legendary` (or one of the 5 `mythic`) card from `cards.json` rather than inventing a new one, unless the act's checklist already calls for a new hero/legendary card.

## 15. Consumables

Defined in `web/src/data/consumables.json`, typed via `ALL_CONSUMABLES` in `web/src/game/itemStore.ts` (see the warning comment there before adding an entry — battle consumables must be added to this file to be drained into `RunState.consumables`; anything else, like a persistent currency, must NOT be). Currently: Health Potion (heal), Second Wind (extra life), Memory Charm (guarantees a reachable memory fragment). Sold at merchant nodes and the hub shop.

Unlike events and quests, growing this pool is **not just data authoring** — each new consumable needs a real mechanical effect wired into the engine (see how `healAmount` / `livesAmount` / `guaranteesFragment` are each read and applied), so treat a new consumable as a small feature, not a JSON entry. Keep the pool's total mechanical variety (not just count) sized so a shop visit offers a genuine choice beyond "heal now vs. later."
