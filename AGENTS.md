...
`npm run test` (vitest) prints an `Unhandled Error` at the end about `browserType.launch: Executable doesn't exist at .../chrome-headless-shell` during browser-cleanup — this is Storybook's `@vitest/browser-playwright` addon looking for a Playwright browser variant that isn't preinstalled in this environment (`web/package.json` pins a `playwright` version newer than the container's pre-installed browser revision, and `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` stops it fetching the missing one). This is expected, not a regression to chase — **but it means the entire `storybook` browser-mode project (every visual/story test) silently did not run.** The `Test Files`/`Tests` summary line only counts the Node project that did run, so "N passed" here is **not** full coverage — a loud banner (`BROWSER-MODE TESTS DID NOT RUN`, from `web/vitestBrowserSkipReporter.ts`) prints right after it to make this unmissable. For a change that touches visual/story-covered code, that coverage only actually runs on CI — rely on the PR's CI run, not a local/cloud-session `npm run test`, to confirm it.
